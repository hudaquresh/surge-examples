...
#------------------------------
    ...
    INPUT:
        print('setrun.final_t:', setrun.final_t) 
    ...
    regions.append([2, 5, rundata.clawdata.t0, rundata.clawdata.tfinal,
                                            70, 75, 17, 22])
    # Mumbai
    regions.append([4, 7, rundata.clawdata.t0, rundata.clawdata.tfinal,
                          72.6, 73, 18.80, 19.15])
    
    # == setgauges.data values ==
    # for gauges append lines of the form  [gaugeno, x, y, t1, t2]
    rundata.gaugedata.gauges.append([1, 72.811790, 18.936508,
                                        rundata.clawdata.t0, 
                                        rundata.clawdata.tfinal])  
    rundata.gaugedata.gauges.append([2, 72.972316, 18.997762,
                                        rundata.clawdata.t0, 
                                        rundata.clawdata.tfinal])  
    rundata.gaugedata.gauges.append([3, 72.819311, 18.818044,
                                        rundata.clawdata.t0, 
                                        rundata.clawdata.tfinal])  
    rundata.gaugedata.gauges.append([4, 72.50, 18.50,
                                        rundata.clawdata.t0, 
                                        rundata.clawdata.tfinal])  
...
def setgeo(rundata):
    ...
    geo_data.sea_level = 0.0  # Due to seasonal swelling of gulf?  
    ...
    # the smaller domains
    #topo_path = os.path.join('../../../../code', 'bathy/')
    topo_path = "/rigel/apam/users/hq2152/bathy/"
    indian_ocean = os.path.join(topo_path, "indian_ocean_n31_s-10_w47_e100.asc")
    ...
    # for moving topography, append lines of the form :   (<= 1 allowed for
    # now!)
...
def set_storm(rundata):
    ...
    # Storm parameters - Parametrized wind models. The choice of wind model
    # menu is below. 
    # 'holland80'               # (type 1)
    # 'holland10'               # (type 2)
    # 'CLE'                     # (type 3)
    ...
    ##storms = load_emanuel_storms(path = os.path.join('/rigel/home/hq2152', 
    ##                                    "Mumbai3_io_ncep_reanalcal.mat"))
    ##print(len(storms))

    ## Load specific storm
    ##1435_plots		1708_old_plots		1708_plots		1708_reanal_plots	618_plots		721_plots		grab_habanero.sh	oldPlots
 
    #mumbai = Storm(path = 'mumbai.storm', file_format="geoclaw")
